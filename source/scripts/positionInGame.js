...
InGamePosition.prototype.entry = function (play) {
    ...
    //values ​​that change with levels (1. 2. 3.)
    let presentLevel = this.level < 11 ? this.level : 10;
    console.log("Level: " + this.level + " PresentLevel number: " + presentLevel);

    //1. UFO speed
    this.ufoSpeed = this.setting.ufoSpeed + (presentLevel * 7);              //Level 1: 35 + (1*7) = 42,        Level 2: 35 + (2*7) = 49, ...
    //2. Bomb falling speed 
    this.bombSpeed = this.setting.bombSpeed + (presentLevel * 10);           //Level 1: 75 + (1*10) = 85,       Level 2: 75 + (2*10) = 95, ...
    //3. Bomb dropping frequency 
    this.bombFrequency = this.setting.bombFrequency + (presentLevel * 0.05); //Level 1: 0.05 + (1*0.05) = 0.1,  Level 2: 0.05 + (2*0.05) = 0.15 ...
...
InGamePosition.prototype.update = function (play) {
    ...
    //keep spaceship in playing field
    ...
    for (let i = 0; i < bullets.length; i++) {
        ...
        //if a bullet flies out from the canvas it will be cleared
        if (bullet.y < 0) {
            bullets.splice(i--, 1);
        }
    }
    ...
    for (let i = 0; i < this.bombs.length; i++) {
        ...
        //if a bomb falls out of the canvas it will be cleared
